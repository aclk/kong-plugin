...
local function cacheable_request(method, uri, conf)
  ...
  -- end
  return true
  -- ngx.log(ngx.NOTICE, "cacheable_request >>>>>> (method: "..method..")")
...
function CacheHandler:new()
  CacheHandler.super.new(self, "key-auth-redis")
...
function CacheHandler:access(conf)
  ...
  -- kong.log.info("############################  CacheHandler:access  ##############################")
  ...
  kong.log.info("////// cache_key >>> "..cache_key.."//////")
  ...
  if cached_val and cached_val ~= ngx.null then
    ...
    return kong.response.send(200, val.content)
  end
...
function CacheHandler:header_filter(conf)
  ...
  -- kong.log.alert("############################  CacheHandler:header_filter  ##############################")
...
function CacheHandler:body_filter(conf)
  ...
  -- kong.log.alert("############################  CacheHandler:body_filter  ##############################")
